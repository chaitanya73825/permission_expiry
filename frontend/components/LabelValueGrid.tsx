  ...
  return (
    <div className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-6 gap-y-2 break-words">
      {items.map(({ label, subLabel, value }) => (
        <Fragment key={label}>
          <div className="flex flex-col text-github-text-muted">
            <div>{label}</div>
...
export function DisplayValue({ value, isCorrect, expected }: DisplayValueProps) {
  return (
    <div className="flex flex-col gap-2">
      <p className={isCorrect ? "text-green-400" : "text-red-400"}>{value}</p>
      {!isCorrect && expected ? <p className="text-sm text-github-text-muted">Expected: {expected}</p> : null}
    </div>
  );
}
